...
public class Container extends Model {
    ...
    public void launch(String name) throws Exception {
        try {
            ...
            Component toLaunch = components.get(name);
            Set<Class<?>> providedInterfaces = toLaunch.getProvidedInterfaces();
            Class<?> appClass = App.class;
            if(providedInterfaces.contains(appClass)){
                Method main = toLaunch.getClass().getMethod("main");
                main.invoke(toLaunch);
            }
        } catch(Exception e) {
